# Reward of jakarta:sonic_horn — runs as the player who blew the horn. using_item fires every tick
...
execute unless entity @s[tag=horn_blowing] if items entity @s weapon.mainhand minecraft:goat_horn[minecraft:custom_data~{sonic_horn:1b}] run function jakarta:horn/fire
tag @s add horn_blowing
scoreboard players set @s horn_grace 3
advancement revoke @s only jakarta:sonic_horn
