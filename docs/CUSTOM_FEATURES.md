...
| --- | --- | --- | --- | --- |
| **Icarus Wings** | elytra | `recipe/icarus_wings.json` (6 feather + 2 honeycomb + 1 string) | Cheap, fast-breaking (216 dura, half a vanilla elytra), unenchantable elytra. **Melts above y=256** (wax drains durability ~6/tick); warning band y224–256. | `function/icarus/` |
...
`pack/data/jakarta/jukebox_song/` — dynamic registry, needs a **restart** to load. The
audio (`.ogg`, Ogg Vorbis) ships in the resource pack and is registered in
`resourcepack/assets/jakarta/sounds.json`. See `docs/features/music_disc.md`.

| Disc | Base | Source | Track | Length |
| --- | --- | --- | --- | --- |
| **Better Luck Next Time** | music_disc_13 | `/function jakarta:disc/give` | Com Truise — Better Luck Next Time | 2:51 |
...
- **God Book** — admin/utility menu (gamemodes, player teleports, ejector/XP utilities, softcore toggles, ender chest, stash, **Friendly Fire on/off**). **Name-gated** (`@s[name=DoubleDome]` in `book/gate`) — only DoubleDome gets the God Book; everyone else gets the Magic Book. `/function`-only (no recipe — an admin book shouldn't be craftable). The old gear tool-swap rows (pickaxe/axe/shovel/hoe/shears/flint) were removed from this page; the `tool_swap_*` gate functions still exist and remain callable via `/function`.
...
- Chunk-cull CLI: `node scripts/cull_chunks.js --dir <dimPath> --x <int> --z <int> --radius <blocks> [--apply]` — keeps chunks within a radius of a point and drops the rest by zeroing their region/entities/poi table entries (so they regenerate). Defaults to a dry-run; **server must be stopped** and the dimension folder backed up first.
...
`recall_fruit`, `trade_reroller`, `music_disc`, `sky_islands_findings`, `softcore_rewrite`,
